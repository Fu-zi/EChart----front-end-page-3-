...
if __name__ == '__main__':
    ...
    #每页15条 向上取整 算出总页数 , 爬取的页码不超过30页
    page_total = math.ceil(total / 15)
    print('page_total:',page_total)
    if page_total > 30:
        page_total = 30
    print('page_total_finall:', page_total)
    # 所有查询结果
    search_job_result = []
    ...
    for i in range(1, page_total+1):
        ...
        df.to_csv(city+'_'+kind+'_'+'lagou.csv', index=False, encoding='utf-8_sig')

    # ---------------数据分析--------------
    #剔除实习岗位的招聘、工作年限无要求或者应届生的当做0年处理、薪资范围需要计算出一个大概的值、学历无要求的当成大专
    # 读取数据
    df = pd.read_csv(city+'_'+kind+'_'+'lagou.csv', encoding='utf-8')
    # 数据清洗,剔除实习岗位
    ...
    df.to_csv(city+'_'+kind+'_'+'lagou_finall.csv', index=False, encoding='utf-8_sig')
    ...
    plt.savefig(city+'_'+kind+'_'+'薪资.png')
    plt.show()
    ...
    plt.savefig(city+'_'+kind+'_'+'公司分布.png')
    plt.show()
    ...
    plt.savefig(city+'_'+kind+'_'+'学历要求.png')
    plt.show()
...
'''
    ...
    # 保存词云图片
    cloud.to_file(city+'_'+kind+'_'+'word_cloud.jpg')
    plt.imshow(cloud)
    plt.axis('off')
    plt.show()
'''
